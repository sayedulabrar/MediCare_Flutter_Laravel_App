...
class DioProvider {
  ...
  Future<dynamic> getUser(String token) async {
    try {
      ...
      print(user);
      print("These are the doctors and details");
      if (user.statusCode == 200 && user.data != '') {
        return json.encode(user.data); // Return the user data as a JSON string
      }
      return null; // Return null if response is empty or status code is not 200
    } catch (error) {
      print("Error fetching user data: $error");
      return null; // Return null or an appropriate error message instead of the exception
    }
  }
